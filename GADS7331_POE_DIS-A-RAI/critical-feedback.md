...
One player wanted **JAI’s output visible permanently** (e.g. a corner feed) and suggested that would help if players **forget Tab** to reopen the AI UI. I was surprised by the **hangar feeling out of scale**, with the avatar reading **too short**, because I had not treated that as a primary issue. The feedback that **Ollama was “overkill”**, with **GPU** named as the cause of **slow replies**, surprised me: it **oversimplified** latency (tokens, prompt size, and UI reveal matter too), but it showed players defaulting to a **hardware explanation**. Most thought-provoking was critique of **missing contextual prompts**: I had treated **sparse on-screen guidance** as intentional so players **must consult JAI** rather than follow omnipresent markers; requests for **visual wayfinding** therefore clashed with that premise. I was also surprised when a player chose to ask JAI a **controls or how-to-play** question rather than a **narrative** one—I had **not thought to test** that path deliberately, and it was **interesting to explore**: it exposed how easily JAI could **over-explain progression** unless the prompt **separated mechanical help from story** (addressed later in `prompts-used.md` Iteration 6). **Neither player** mentioned **jerky elevator motion**, which I had expected to notice—so that felt **less salient** to them than to me.

---

## What did I ignore or choose not to implement?

I did **not implement** inference on a **remote GPU** “for speed.” For a **university project**, that implies **cost**, **time**, and **major re-engineering**, plus **technical limits** before hand-in. I did **not replace** the LLM with a **database of scripted lines** for instant “AI” replies—that would **contradict** **dynamic** dialogue tied to player input. I did **not add** blanket **contextual prompts** on every interactable: that would **undermine** **environmental reading** and **JAI-led guidance** in favour of a constant HUD tutorial layer.

I also **didn't implement** the suggested **permanent corner feed** for **JAI’s output**— as it would make the player's screen feel **too bulky** and would **block too much of the screen** during play. Instead I added a **small Tab UI indicator on the left side of the screen**: a compromise that nudges players who **forget** they can open JAI, without keeping a full transcript **always** on screen.
...
The refinements I **prioritised** were the items that **raise clarity and comfort** without **replacing** the player’s need to **talk to JAI**: **Shift sprint**, **Enter** to **advance dialogue** and to **send** chat (reducing mouse friction), fixing **E** firing **while typing** near interactables, **clearer opening guidance** on **how to interact** with objects, a **more legible first-room teaching beat** (so controls are learned before pressure rises), **subtle hangar wayfinding** (enough orientation, not a glowing objective trail), review of **hangar scale / proportions**, a **small left-side Tab indicator** (instead of a **full permanent corner feed** for JAI output), **faster on-screen reveal** of AI text, **prompt controls branch** for how-to-play questions, and **tighter colliders / snapping** for **power cells**. These map to the post-event log in `feedback-summary.md` and are recorded in `refinements-changes.md` as each change is implemented.

**Which feedback did I decline, and why?**  
I **declined** three families of suggestion: **remote GPU** hosting for inference, a **database** of **pre-written** “AI” lines for speed, and **blanket contextual prompts** on every interactable. I also **declined the full permanent corner feed** for JAI text in favour of the **lighter Tab indicator**, to avoid **screen obstruction**. **Remote GPU** fails on **time, cost, re-engineering**, and **technical risk** for a **university** submission. The **database** approach would **gut** the reason for using an **LLM**—responses would stop being **genuinely responsive** to free-form player input. **Omnipresent prompts** would **collapse** the design premise that **JAI** is the **unreliable guide** players must interrogate. I also did **not** treat “**replace Ollama entirely**” as an automatic action item: the critique flags **latency**, but the proportionate response here is **local tuning** (prompt length, generation limits, UI pacing), not a **new hosting architecture**.
